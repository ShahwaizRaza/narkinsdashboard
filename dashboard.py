...
def calculate_all_metrics(df, today):
    ...
    if total_branch > 0:
        metrics['month_by_branch']["Contribution %"] = (
            metrics['month_by_branch']["Total Sales"] / total_branch * 100
        ).round(2)
    
    total_cat = metrics['month_by_category']["Total Sales"].sum()
    if total_cat > 0:
        metrics['month_by_category']["Contribution %"] = (
            metrics['month_by_category']["Total Sales"] / total_cat * 100
        ).round(2)
...
def get_category_top_10(df, category_name):
    ...
    filtered = df[df['Category'].str.contains(category_name, case=False, na=False)].copy()
    filtered["Product"] = filtered["Product Name"].str.split("|").str[0].str.strip()
    
    return (
        filtered.groupby("Product")[["SOLD QTY", "Total Sales"]]
        .sum().sort_values(by="Total Sales", ascending=False).head(10).reset_index()
    )
...
with tab3:
    ...
    with col1:
        ...
        top_ns = get_category_top_10(df, "NARMIN STITCHED")
        st.dataframe(top_ns, height=400, use_container_width=True, hide_index=True)
    
    with col2:
        st.subheader("🌿 Top 10 Cotton")
        top_cotton = get_category_top_10(df, "COTTON")
        st.dataframe(top_cotton, height=400, use_container_width=True, hide_index=True)
    
    col3, col4 = st.columns(2)
    
    with col3:
        st.subheader("🎨 Top 10 Blended")
        top_blend = get_category_top_10(df, "BLENDED")
        st.dataframe(top_blend, height=400, use_container_width=True, hide_index=True)
    
    with col4:
        st.subheader("❄️ Top 10 Winter")
        top_winter = get_category_top_10(df, "WINTER")
        st.dataframe(top_winter, height=400, use_container_width=True, hide_index=True)
...
# ----------------------------------------------------
st.divider()
st.caption(f"🕐 Last Updated: {datetime.now().strftime('%Y-%m-%d %I:%M:%S %p')} | 📱 Mobile & Desktop Optimized | 🔄 Data cached for 10 minutes")
